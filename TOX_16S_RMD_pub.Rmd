...
```{r, include=FALSE}
mothur_b_data= import_mothur(mothur_shared_file ="TOX2_16S.shared", mothur_constaxonomy_file ="TOX2_16S.taxonomy") #load OTU and taxonomy tables from mothur files
...
moth_merge = merge_phyloseq(mothur_b_data, map) #merge mothurdata with the metadata
...
samp = prune_taxa(taxa_sums(samp) > 0, samp) #removes 65 OTUs, likely represented in check and blank samples
...
min(cov_df$goods) #0.9942
```

```{r, include=FALSE}
moth_sub=subset_samples(moth_merge, Type == "sample") #remove check samples 
moth_sub=prune_taxa(taxa_sums(moth_sub) > 0, moth_sub) #this removes 65 Otus, likely those specific to the check samples and controls
moth_sub = prune_taxa(taxa_sums(moth_sub) > 1, moth_sub) #remove singletons
sample_sums_pruned=as.data.frame(sample_sums(moth_sub))

total_reads=colSums(sample_sums_pruned) #10579899 
avg_reads=round(mean(sample_sums(moth_sub))) #78954
...
```{r, include=FALSE}
#look at distribution of libraries 
...
min=round(min(sample_sum_TOX$sum)) #The smallest library contains 26780 sequences
max=round(max(sample_sum_TOX$sum)) #The largest library contains 196590 sequences
avg=round(mean(sample_sum_TOX$sum)) #The average library size is 78954 sequences
...
TOX_Order_fluid = TOX_Order %>%
  filter(Sample_Type == "fluid")
TOX_Order_fluid$Tax_rank = "Order"
TOX_Order_fluid$Tax_ID = TOX_Order_fluid$Order
Order_fluid = TOX_Order_fluid %>%
  select(OTU, Sample, Abundance, group, Donor, Tax_rank, Tax_ID)

ggplot(TOX_Order_fluid, aes(x=group, y=Abundance, fill = Tax_ID)) + geom_bar(stat = "identity") + scale_fill_manual(values = phylum_colors)+ theme_bw() + theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8))+ theme_bw()+ theme(legend.position="bottom") +guides(fill=guide_legend(ncol=5)) 
...
nb.cols=50
large_expanded_cols=colorRampPalette(phylum_colors)(nb.cols) #make sure there are enough colors for the number of taxa present in the merged dataframe

#add classifiers to taxa names so they can will be easier to distinguish in the merged dataframe and in the merged plot
phylum_fluid$Tax_ID = sub("^", "p_", phylum_fluid$Tax_ID)
Class_fluid$Tax_ID = sub("^", "c_", Class_fluid$Tax_ID)
Genus_fluid$Tax_ID = sub("^", "g_", Genus_fluid$Tax_ID)
...
fluid_relabund_graphing$library = "Bact"

#plot
a=ggplot(fluid_relabund_graphing, aes(x=group, y=Abundance, fill = Tax_ID)) + geom_bar(stat = "identity") + scale_fill_manual(values = large_expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) + facet_wrap(~Tax_rank, scales = "free_x", nrow = 1)+ theme(legend.position="bottom") +guides(fill=guide_legend(ncol=3))

#determine the avergage number of phyla present between donors
avg_phyla=phylum_fluid %>%
  group_by(Donor) %>%
  summarise(num_phyla = length(unique(Tax_ID)))
bact_df_avg_phyla= round(mean(avg_phyla$num_phyla),0)
bact_df_phyla_min= min(avg_phyla$num_phyla)
bact_df_phyla_max = max(avg_phyla$num_phyla)
...
fluid_genus_abund_avgs = Genus_fluid %>%
  group_by(Tax_ID) %>%
  summarise(avg_phyla_abund = mean(Abundance)*100) %>%
  arrange(desc(avg_phyla_abund))

```

Fluid relative abundance: Phylum, Class, and Genus

```{r, echo=FALSE}
a
```

There are an average of `r bact_df_avg_phyla` 16S phyla present in decomposition fluid samples, with a range of `r bact_df_phyla_min` - `r bact_df_phyla_max` phyla. 
...
TOX_phylum = TOX %>%
  ...
  psmelt() %>%
  filter(Abundance > 0.05) %>%
  arrange(Phylum) #calculate relative abundance of each phylum and remove phyla less than 5%
...
phylum_soil_sum_zero_long = phylum_soil_sum_zero %>%
  gather(key = "Tax_ID", value = "sum_phylum", "Acidobacteria":"Verrucomicrobia") %>%
  arrange(Donor,Timepoint) #change the dataset back to long format so we can calculate the difference

#Quick proof of concept that abundance values are being calculated correctly:
#ggplot(subset(TOX_phylum, Donor %in% c("TOX001")), aes(x = group, y = Abundance, fill = phylum)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 

#ggplot(subset(phylum_soil_sum, Donor %in% c("TOX001")), aes(x = Timepoint, y = sum_phylum, fill = Tax_ID)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 
...
TOX_Class = TOX %>%
  ...
  psmelt() %>%
  filter(Abundance > 0.05) %>%
  arrange(Class) #calculate relative abundance of each class and remove classes less than 5%
...
class_soil_sum_zero_long = class_soil_sum_zero %>%
  gather(key = "Tax_ID", value = "sum_class", "Acidobacteriia":"Verrucomicrobiae") %>%
  arrange(Donor,Timepoint) #change the dataset back to long format so we can calculate the difference

#Quick proof of concept that abundance values are being calcualted correctly:
#ggplot(subset(TOX_Class, Donor %in% c("TOX001")), aes(x = group, y = Abundance, fill = Class)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 

#ggplot(subset(Class_soil_sum, Donor %in% c("TOX001")), aes(x = Timepoint, y = sum_class, fill = Tax_ID)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 
...
Soil Class: median max absolute difference
```{r, echo=FALSE}
...
TOX_genus = TOX %>%
  ...
  arrange(Genus)
 # filter(Abundance > 0.05) %>%
   #calculate relative abundance of each genus and remove genuses less than 5%
...
#genus_soil_sum_zero$01D2Z36_ge
#genus_soil_sum_zero$Zoogloea

genus_soil_sum_zero_long = genus_soil_sum_zero %>%
  gather(key = "Tax_ID", value = "sum_genus", "01D2Z36_ge":"Zoogloea") %>%
  arrange(Donor,Timepoint) #change the dataset back to long format so we can calculate the difference
...
#Quick proof of concept that abundance values are being calcualted correctly:
#ggplot(subset(TOX_genus, Donor %in% c("TOX001")), aes(x = group, y = Abundance, fill = genus)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 

#ggplot(subset(genus_soil_sum, Donor %in% c("TOX001")), aes(x = Timepoint, y = sum_genus, fill = Tax_ID)) + geom_bar(stat = "identity") + scale_fill_manual(values = expanded_cols) + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6)) + guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1))+xlab("Sample")+ylab("Relative Abundance") 
...
```{r, include=FALSE}
alphadiv = read_excel("TOX2_bact_alphadiv.xlsx") # this will load the file that is generated by the code below, which calculated Chao1, Inverse Simpson, and Richness:
...
```{r, include=FALSE}
alphadiv_soil=filter(alphadiv, Sample_Type == "soil")
alphadiv_fluid=filter(alphadiv, Sample_Type == "fluid")

#reshape DF of fluid samples only
...
#reshape DF of soil samples only
alphadiv_soil_wide = alphadiv_soil %>% select(SampleID, mean, measure, Donor, ADH, Total_ADH, Season,Sex, Age, BMI, Cancer, Diabetes, Cardio, Neuro, Neuro, Pneumonia)
alphadiv_soil_wide= dcast(alphadiv_soil_wide, SampleID + Donor + ADH + Total_ADH + Season + Sex + Age + BMI + Cancer +Diabetes + Cardio + Neuro + Neuro + Pneumonia ~ measure, value.var="mean")
...
alphadiv_soil_clean = alphadiv_soil %>%
  ...
  filter(Timepoint != "P") %>%
  filter(Timepoint != "Q")
alphadiv_soil_clean$Timepoint=as.factor(alphadiv_soil_clean$Timepoint, labels = c("Control","0 ADH","1500 ADH","3000 ADH","4500 ADH","6000 ADH","7500 ADH", "8500 ADH","13000 ADH"))
...
#ggplot(alphadiv_soil_clean, aes(x=Timepoint, y=mean, fill=Timepoint))+ geom_violin(trim=FALSE)+geom_boxplot(width=0.1, fill="white")+scale_x_discrete(labels=c("Control","0 ADH","1500 ADH","3000 ADH","4500 ADH","6000 ADH","7500 ADH", "8500 ADH","13000 ADH"))+theme_bw()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+scale_fill_manual(values=colpalette2, guide=FALSE)+facet_grid(~measure, scales = "free_y")
  
colnames(alphadiv_soil_wide)[16]="InvSimp"

#compare fluid and soil alpha div:
#take the min alpha div value across each soil sample from each donor:
  min_chao_soil = alphadiv_soil_wide %>%
    group_by(Donor) %>%
    summarise(min_chao_soil = min(Chao1))
  fluid_soil_alpha_comp = alphadiv_fluid_wide %>%
    left_join(min_chao_soil, by= "Donor")
  fluid_soil_alpha_comp = fluid_soil_alpha_comp %>%
    mutate(chao_soil_div_fluid = round((min_chao_soil/Chao1), digits=0))
  
  min_invs_soil = alphadiv_soil_wide %>%
    group_by(Donor) %>%
    summarise(min_invsimp_soil = min(InvSimp))
  fluid_soil_alpha_comp = fluid_soil_alpha_comp %>%
    left_join(min_invs_soil, by= "Donor")
  fluid_soil_alpha_comp = fluid_soil_alpha_comp %>%
    mutate(InvSimp_soil_div_fluid = round((min_invsimp_soil/InvSimp), digits=0))
  
test = fluid_soil_alpha_comp %>% select(Donor, Chao1, InvSimp, min_chao_soil, min_invsimp_soil)%>%
  gather(key= measure, value = value, Chao1:min_invsimp_soil)

test$alpha= c("Chao1", "Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1"   ,"Chao1","Chao1", "Chao1","InvSimp",  "InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp" ,"InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","Chao1","Chao1"  ,"Chao1" ,"Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","Chao1","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp","InvSimp")

ggplot(test, aes(x=measure, y=value, color=measure))+geom_boxplot()+theme_bw()+scale_color_manual(values=colpalette2, guide=FALSE)+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+facet_wrap(~alpha, scales = "free")
  
  
fluid_soil_chao_table = fluid_soil_alpha_comp %>% select(Donor, Season, BMI, Chao1, min_chao_soil, chao_soil_div_fluid, InvSimp, min_invsimp_soil, InvSimp_soil_div_fluid)

colnames(fluid_soil_chao_table) = c("Donor", "Season", "BMI", "Chao1 Fluid", "Min Chao1 Soil", "Magnitude of Difference - Richness", "Inverse Simpson Fluid", "Min Inverse Simpsion Soil", "Magnitude of Difference - Inverse Simpson")
 
mean(fluid_soil_alpha_comp$chao_soil_div_fluid)
median(fluid_soil_alpha_comp$chao_soil_div_fluid)
  
mean(fluid_soil_alpha_comp$InvSimp_soil_div_fluid)
median(fluid_soil_alpha_comp$InvSimp_soil_div_fluid)
```

### Fluid - Soil Alpha diversity comparison:

```{r, echo=FALSE}
kable(fluid_soil_chao_table, digits = 2)
```
...
#color by number of drugs detected in blood serum
...
```{r, echo=FALSE}
BMI_cat = metadata %>% filter(Per_ADH == "1") %>% select(Donor, BMI_level)
...
BMI did not significantly alter bacterial richness (p= `r round(BMI, 3)`) or diversity (p= `r round(BMI2,3)`) in fluid samples.

Fluid alpha diversity and Total ADH:
```{r, include=FALSE}
#test: does Chao1 and/or inverse simpson in fluid samples vary due to the total ADH to complete active decomposition?
...
alphadiv_fluid_wide %>% filter(Total_ADH <= 5000) %>% summarise(med_chao1_5000 = median(Chao1))
alphadiv_fluid_wide %>% filter(Total_ADH <= 5000) %>% summarise(mean_chao1_5000 = mean(Chao1))
alphadiv_fluid_wide %>% filter(Total_ADH >= 10000) %>% summarise(med_chao1_5000 = median(Chao1))
...
```{r, echo=FALSE}
#plot chao1, richness, and inverse simpson and color by total ADH to complete active decomposition
...
```{r, echo=FALSE}
#plot chao1, richness, and inverse simpson and color by season of placement
...
```{r, include=FALSE}
#test: does Chao1 and/or inverse simpson in fluid samples vary due to the season of placement?
...
Season did not significantly alter bacterial community richness (p= `r round(Season, 3)`) or diversity (p= `r round(Season2,3)`) in fluid samples.

Fluid alpha diversity and Sex:

```{r, echo=FALSE}
#plot Chao1, richness, and inverse simpson colored by biological sex of the subject
...
```{r, include=FALSE}
#test: does Chao1 and/or inverse simpson in fluid samples vary due to biological sex of the subject?
...
```{r, echo=FALSE}
#plot Chao1, richness, and inverse simpson colored by age of the subject
ggplot(alphadiv_fluid, aes(x=Age, y=mean)) + geom_point() +geom_smooth(method = "lm") + facet_wrap(~measure, ncol = 3, scales = "free")+theme_bw()
```


```{r, include=FALSE}
#test: does Chao1 and/or inverse simpson in fluid samples vary due to age of the subject?
...
The number of drugs detected in blood serum did not significantly alter richness (p= `r round(drugs, 3)`) or diversity (p= `r round(drugs2,3)`) in fluid samples (bacteria).

### Fluid alpha diversity x illnesses
```{r, include=FALSE}
#test: does Chao1 and/or inverse simpson in fluid samples vary due to different diseases reported as contributing to death of the subject?
...
resp_r=lm(Chao1~Neuro, data=alphadiv_fluid_wide)
summary(resp_r)
resp_i=lm(InvSimp~Neuro, data=alphadiv_fluid_wide)
summary(resp_i)
...
```{r, include=FALSE}
test=alphadiv_fluid_wide %>% select(SampleID, Chao1, InvSimp, Donor, Cancer, Diabetes, Cardio, Neuro, Neuro, Pneumonia)
...
```{r, include=FALSE}
#plot chao1 in soil samples over time (up to 6000 ADH) for subjects with and without cancer (barplot with wilcoxon results)
#Keep: control (A), 0 (B), 1500 (G), 3000 (H), 4500 (I), 6000 (J) for cancer barplot
library("plotrix")
chao_sub = alphadiv_soil %>%
  filter(measure == "Chao1") %>%
  filter(Timepoint != "D" & Timepoint != "E"& Timepoint != "F"& Timepoint != "K"& Timepoint != "L"& Timepoint != "M"& Timepoint != "N"& Timepoint != "O"& Timepoint != "P"& Timepoint != "Q"& Timepoint != "NA") %>% filter(SampleID != "TOX002-1500A") %>% filter(SampleID != "TOX002-1500B")
colnames(chao_sub)[3] = "chao1" #rename the "mean" column to chao1

chao_sub$Cancer = as.factor(chao_sub$Cancer)
chao_sub$Timepoint = factor(chao_sub$Timepoint, labels = c("Control","0 ADH","1500 ADH", "3000 ADH", "4500 ADH", "6000 ADH"))

chao_cancer_mean = chao_sub %>%
  group_by(Timepoint,Cancer)  %>% 
  summarize(se=std.error(chao1), chao1= mean(chao1))


theme_set(theme_gray(base_size = 15))

chao_cancer_bar=ggplot(chao_cancer_sub, aes(x=Timepoint, y=chao1, color = Cancer)) +
  geom_col(data = chao_cancer_mean, position = position_dodge(0.8), width = 0.7, fill = "white")+
  geom_jitter(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8)) + 
  geom_errorbar(aes(ymin = chao1-se, ymax = chao1+se), data = chao_cancer_mean, width = 0.2, position = position_dodge(0.8))+
  scale_color_manual(values = c("#999999", "#E69F00")) +
  theme_bw()+
  theme(legend.position = "top")+
  labs(x="Accumulated Degree Hours (ADH)") +stat_compare_means(method="wilcox.test", label="p.format",  hide.ns=TRUE, label.y=4250, show.legend = FALSE)
```


```{r, include=FALSE}
##plot chao1 in soil samples over time (up to 6000 ADH) for subjects with and without respiratory illnesses (barplot with wilcoxon results)
chao_sub$Respiratory = as.factor(chao_sub$Respiratory)

chao_Respiratory_mean = chao_sub %>%
  group_by(Timepoint,Respiratory)  %>% 
  summarize(se=std.error(chao1), chao1= mean(chao1))

theme_set(theme_gray(base_size = 15))

chao_Respiratory_bar=ggplot(chao_sub, aes(x=Timepoint, y=chao1, color = Respiratory)) +
  geom_col(data = chao_Respiratory_mean, position = position_dodge(0.8), width = 0.7, fill = "white")+
  geom_jitter(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8)) + 
  geom_errorbar(aes(ymin = chao1-se, ymax = chao1+se), data = chao_Respiratory_mean, width = 0.2, position = position_dodge(0.8))+
  scale_color_manual(values = c("#999999", "#E69F00")) +
  theme_bw()+
  theme(legend.position = "top")+
  labs(x="Accumulated Degree Hours (ADH)") +stat_compare_means(method="wilcox.test", label="p.format",  hide.ns=TRUE, label.y=4250, show.legend = FALSE)
...
```{r, include=FALSE}
#plot chao1 in soil samples over time (up to 6000 ADH) for subjects with and without neurological illnesses (barplot with wilcoxon results)
chao_sub$Neuro = as.factor(chao_sub$Neuro)

chao_Neuro_mean = chao_sub %>%
  group_by(Timepoint,Neuro)  %>% 
  summarize(se=std.error(chao1), chao1= mean(chao1))

theme_set(theme_gray(base_size = 15))

chao_Neuro_bar=ggplot(chao_sub, aes(x=Timepoint, y=chao1, color = Neuro)) +
  geom_col(data = chao_Neuro_mean, position = position_dodge(0.8), width = 0.7, fill = "white")+
  geom_jitter(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8)) + 
  geom_errorbar(aes(ymin = chao1-se, ymax = chao1+se), data = chao_Neuro_mean, width = 0.2, position = position_dodge(0.8))+
  scale_color_manual(values = c("#999999", "#E69F00")) +
  theme_bw()+
  theme(legend.position = "top")+
  labs(x="Accumulated Degree Hours (ADH)") +stat_compare_means(method="wilcox.test", label="p.format",  hide.ns=TRUE, label.y=4250, show.legend = FALSE)
...
alphadiv_soil$ADH = as.numeric(alphadiv_soil$ADH)
chao_sub_5000 = alphadiv_soil %>%
  filter(measure == "Chao1") %>% filter(ADH <= 5000) %>% filter(Trt == "donor")%>% filter(SampleID != "TOX002-1500A") %>% filter(SampleID != "TOX002-1500B")
ggplot(chao_sub_5000, aes(ADH, mean, group=Neuro, color=Neuro)) + geom_point(size=3) + geom_smooth(method = lm) + theme_bw()

#combine the chao1 cancer, respiratory illness, and neurological illness plots into one image 
library(ggpubr)
ggarrange(chao_cancer_bar, chao_Respiratory_bar, chao_Neuro_bar, labels = c("A","B","C"), nrow = 1)
```
...
```{r, include=FALSE}
#rareify randomly to the smallest library size
...
Bray_fluid=plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="Donor", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color = Donor), alpha = 0.7, size=4)+geom_point(colour="grey90", size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()

#fluid diversity by BMI and donors labeled
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="BMI", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=BMI), alpha = 1, size=4)+geom_text(aes(label=Donor),hjust=.5, vjust=2)+theme_bw()

#fluid diversity by Age
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="Age", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=Age), alpha = 1, size=4)+theme_bw()

#Age x Season
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="Age", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=Age,shape=Season), alpha = 1, size=4)+theme_bw()
```
...
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="num_drugs", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=num_drugs), alpha = 1, size=4)+theme_bw()
```
...
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="Total_ADH", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=Total_ADH), alpha = 1, size=4)+theme_bw()
```
...
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="Season", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=Season), alpha = 1, size=4)+ scale_color_manual(values = colpalette2)+theme_bw()
```
...
```{r, echo=FALSE}
#BMI x Season
plot_ordination(physeq=TOX_scale_fluid, ordination=TOX1_bray_fluid, color="BMI", title= "PCoA of Fluid Fungal Communitites: Bray (001-020)") + geom_point(aes(color=Weight,shape=Season), alpha = 1, size=4)
```
...
perm4=adonis(bray_fluid~Total_ADH+Cancer+Diabetes+Cardio+Neuro+Neuro, data=trts.fluid, permutations=1000, by="margin")
...
perm12 = adonis(bray_fluid~Resp.y, data=trts.fluid, permutations=1000, by="margin")
...
PERMANOVA:bray_fluid~Total_ADH+Cancer+Diabetes+Cardio+Resp+Neuro
```{r, echo=FALSE}
...
PERMANOVA: soil control ~ season
```{r, echo=FALSE}
kable(soil_con_perm$aov.tab, digits = 2)
```

Beta dispersion: Soil control ~ season
```{r, echo=FALSE}
#test beta-dispersion between seasons 
ggplot(beta.df, aes(x=Season, y=dispersion, fill=Season)) +  geom_violin(trim=FALSE)+geom_boxplot(width=0.1, fill="white")+ scale_fill_brewer(palette="Dark2") + theme_minimal()
```


```{r, echo=FALSE}
kable(beta_permu$tab, digits = 2)
```
...
plot_ordination(physeq=TOX_scale_soil, ordination=TOX1_bray_soil, color="Per_ADH", title= "PCoA of TOX1 Soil Bacterial Communitites: Bray") + geom_point(aes(color = Per_ADH), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_viridis() + theme_bw()

plot_ordination(physeq=TOX_scale_soil, ordination=TOX1_bray_soil, shape="Trt", color="Donor", title= "PCoA of TOX1 Soil Bacterial Communitites: Bray") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))
...
```{r, echo=FALSE}
Bray_soil=plot_ordination(physeq=TOX_scale_soil, ordination=TOX1_bray_soil, color="Timepoint", shape="Trt", title= "PCoA of TOX1 Soil Bacterial Communitites: Bray") + geom_point(aes(color = Timepoint), alpha = 0.7, size=4)+geom_point(colour="grey90", size=1.5) + scale_color_manual(values = colpalette2,labels=c("Control", "0","250","500","1000","1500", "3000", "4500", "6000", "7500", "8500", "11500", "13000", "15500", "17500","18500", "NA"))
Bray_soil
```

Soil communities PCoA: Donor and ADH

```{r, echo=FALSE}
plot_ordination(physeq=TOX_scale_soil, ordination=TOX1_bray_soil, color="Per_ADH", shape="Donor", title= "PCoA of TOX1 Soil Bacterial Communities: Bray") + geom_point(aes(color = Per_ADH), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+scale_shape_manual(values=c(0:20))+scale_color_viridis()+theme_bw()
```
...
```{r, echo=FALSE}
plot_ordination(physeq=TOX_scale_soil, ordination=TOX1_bray_soil, color="Per_ADH", shape="Donor", title= "PCoA of TOX1 Soil Fungal Communities: Bray") + geom_point(aes(color = Per_ADH), stroke=1.5, alpha = 1.5, size=2)+geom_point(size=1.5)+scale_shape_manual(values=c(0:20))+scale_color_viridis()+theme_bw()
```
...
### Soil: all timepoints 
```{r, include=FALSE}
#run beta dispersion with all soil samples across all timepoints
homo=with(trts.soil,betadisper(bray_soil, Donor)) #3.5277 ; p=0.001 ***
a=permutest(homo)
donor=a$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Season)) #6.6616 ; 0.002 ***
b=permutest(homo)
Season=b$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Sector)) #5.9694  ; 0.001 ***
c=permutest(homo)
Sector=c$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Trt)) #22.359 ;  0.001 ***
d=permutest(homo)
Trt=d$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Sex)) #0.1682 ; 0.687
e=permutest(homo)
Sex=e$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Cancer)) #4.105;   0.046 *
f=permutest(homo)
cancer=f$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Diabetes)) #2.0677  ; 0.152
g=permutest(homo)
diabetes=g$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Cardio)) #0.7732  ; 0.377
h=permutest(homo)
cardio=h$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Respiratory)) #17.51 ; 0.001 ***
i=permutest(homo)
resp=i$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Neuro)) #3.842  ; 0.052
j=permutest(homo)
neuro=j$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Pneumonia)) #2.3253  ;  0.146
k=permutest(homo)
pneu=k$tab[1,4:6]
homo=with(trts.soil,betadisper(bray_soil, Timepoint)) #7.3175  ;  0.001 ***
l=permutest(homo)
time=l$tab[1,4:6] #need to rerun with a cutoff @ 8500 ADH
homo=with(trts.soil,betadisper(bray_soil, BMI_level))
m=permutest(homo)
bmi=m$tab[1,4:6]

test=rbind(donor,Season,Sector,Trt,Sex,cancer,diabetes,cardio,resp,neuro,pneu,time,bmi)
test$Variable=c("Donor","Season","Sector","Treatment","Sex","Cancer","Diabetes","Cardiovascular","Respiratory","Neurological","Pneumonia","Timepoint","BMI")
test2=test[,c(4,1,2,3)]
```


Betadispr results (All timepoints):
```{r, echo=FALSE}
kable(test2, digits = 3)
```

We will want to investigate the following variables with pairwise tukey tests: Donor (to compare to 16S), Sector, Neuro, and Timepoint. 

```{r, include=FALSE}
homo=with(trts.soil,betadisper(bray_soil, Donor)) #make an object of the beta dispersion analysis
plot(homo) #view dispersion
boxplot(homo) #view dispersion values as a box plot
permutest(homo) #similar to anova, test for significant differences in dispersion (sig. result = reject null that dispersion among groups is the same-aka they are different); if they are different, we can run TukeyHSD to see which groups significantly differ from each other
...
homo.dist=as.data.frame(homo$distances) #we want to make a box plot and add the tukey results, so we need the distance values from the beta dispersion object (here it is homo)
...
ggplot(homo.df, aes(x=Donor, y=dispersion)) +geom_boxplot(aes(fill=BMI_level), position=position_dodge(0.8)) + geom_text(data = final, aes(x = Donor, y = .8, label = Letters),size=5) + theme_bw()+theme(axis.text.x = element_text(angle = 90,vjust = .5))
```
...
```{r, include=FALSE}
mean_disp = homo.df %>% group_by(Donor) %>% summarise(mean_disp = mean(dispersion))
mod= lmer(dispersion~ADH*BMI_level+(1|Donor), data=homo.df)
aov=anova(mod)

homo.df_5000 = homo.df %>% filter(ADH <= 5000)

ggplot(homo.df, aes(x=ADH,y=dispersion,color=BMI_level))+geom_point()+geom_smooth(method = "lm")+theme_bw()

ggplot(homo.df_5000, aes(x=ADH,y=dispersion,color=BMI_level))+geom_point()+geom_smooth(method = "lm")+theme_bw()

homo.df_single = homo.df %>% filter(Per_ADH == 1) %>% left_join(mean_disp, by = "Donor")
mod2 = lm(mean_disp~BMI_level, data = homo.df_single)
anova(mod2)
```

Beta dispersion pairwise comparisons: Timepoint (through 8500 ADH)

```{r, include=FALSE}
#test beta disperion with soil samples only through 8500 ADH as replication is low after this timeperiod due to differences in total ADH to reach active decomposition 
...
```{r, include=FALSE}
mod= lmer(dispersion~ADH+(1|Donor), data=homo.df)
aov=anova(mod)
```


```{r, include=FALSE}
#run PERMANOVAs with soil samples
...
plot_ordination(physeq=TOX_scale_soil_donor, ordination=TOX1_bray_soil_donor, color="BMI_level", title= "PCoA of TOX1 Soil Fungal Communitites: Bray") + geom_point(aes(color = BMI_level), alpha = 4, size=4) + stat_ellipse(aes(color = BMI_level, group = BMI_level))+ theme_bw()

plot_ordination(physeq=TOX_scale_soil_donor, ordination=TOX1_bray_soil_donor, color="Per_ADH", shape="BMI_level", title= "PCoA of TOX1 Soil Fungal Communitites: Bray") + geom_point(aes(color = Per_ADH), alpha = 4, size=4) + scale_color_viridis() + theme_bw()

perm=adonis(bray_soil_donor~ADH*BMI_level, data=trts.soil_donor, permutations=1000, strata = trts.soil_donor$Donor)

homo=with(trts.soil_donor,betadisper(bray_soil_donor, BMI_level)) #5.914 ; p=0.003
...
x = data.frame(sample_data(TOX_scale_soil_0_6000))

bray_pcoa_soil_0_6000 = ordinate(physeq = TOX_scale_soil_0_6000, method = "PCoA", distance = "bray")

plot_ordination(physeq=TOX_scale_soil_0_6000, ordination=bray_pcoa_soil_0_6000, color="Per_ADH", title= "PCoA of TOX1 Soil Bacterial Communitites: Bray") + geom_point(aes(color = Per_ADH), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_viridis() + theme_bw()
```
...
```{r}
plot_ordination(physeq=TOX_scale_soil_control, ordination=bray_pcoa_soil_control, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: controls") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))
```
...
```{r}
plot_ordination(physeq=TOX_scale_soil_0, ordination=bray_pcoa_soil_0, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 0 ADH") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))

plot_ordination(physeq=TOX_scale_soil_0, ordination=bray_pcoa_soil_0, color="Season", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 0 ADH") + geom_point(aes(color = Season), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()
```
...
```{r}
plot_ordination(physeq=TOX_scale_soil_1500, ordination=bray_pcoa_soil_1500, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 1500 ADH") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))

plot_ordination(physeq=TOX_scale_soil_1500, ordination=bray_pcoa_soil_1500, color="Age_cat", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 1500 ADH") + geom_point(aes(color = Age_cat), stroke=1.5, alpha = 1.5, size=4) +theme_bw()
```
...
```{r}
plot_ordination(physeq=TOX_scale_soil_3000, ordination=bray_pcoa_soil_3000, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 3000 ADH") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))

plot_ordination(physeq=TOX_scale_soil_3000, ordination=bray_pcoa_soil_3000, color="Age_cat", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 3000 ADH") + geom_point(aes(color = Age_cat), stroke=1.5, alpha = 1.5, size=4) +theme_bw()

plot_ordination(physeq=TOX_scale_soil_3000, ordination=bray_pcoa_soil_3000, shape = "BMI_level", color="BMI_level", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 3000 ADH") + geom_point(aes(color = BMI_level), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()

plot_ordination(physeq=TOX_scale_soil_3000, ordination=bray_pcoa_soil_3000, shape = "Season", color="Sector", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 3000 ADH") + geom_point(aes(color = Sector), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()
```
...
perm2=adonis(bray_soil_3000~BMI, data=trts.soil_3000, permutations=1000, by="margin")
...
```{r}
plot_ordination(physeq=TOX_scale_soil_4500, ordination=bray_pcoa_soil_4500, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 4500 ADH") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))

plot_ordination(physeq=TOX_scale_soil_4500, ordination=bray_pcoa_soil_4500, color="Age_cat", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 4500 ADH") + geom_point(aes(color = Age_cat), stroke=1.5, alpha = 1.5, size=4) +theme_bw()

plot_ordination(physeq=TOX_scale_soil_4500, ordination=bray_pcoa_soil_4500, shape = "BMI_level", color="BMI_level", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 4500 ADH") + geom_point(aes(color = BMI_level), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()

plot_ordination(physeq=TOX_scale_soil_4500, ordination=bray_pcoa_soil_4500, shape = "Season", color="Sector", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 4500 ADH") + geom_point(aes(color = Sector), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()
```
...
perm2=adonis(bray_soil_4500~BMI, data=trts.soil_4500, permutations=1000, by="margin")
...
```{r}
plot_ordination(physeq=TOX_scale_soil_6000, ordination=bray_pcoa_soil_6000, color="Donor", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 4500 ADH") + geom_point(aes(color = Donor), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5) + scale_color_manual(values = colpalette2)+theme_bw()+scale_shape_manual(values=c(8, 19))

plot_ordination(physeq=TOX_scale_soil_6000, ordination=bray_pcoa_soil_6000, color="Age_cat", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 6000 ADH") + geom_point(aes(color = Age_cat), stroke=1.5, alpha = 1.5, size=4) +theme_bw()

plot_ordination(physeq=TOX_scale_soil_6000, ordination=bray_pcoa_soil_6000, shape = "BMI_level", color="BMI_level", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 6000 ADH") + geom_point(aes(color = BMI_level), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()

plot_ordination(physeq=TOX_scale_soil_6000, ordination=bray_pcoa_soil_6000, shape = "Season", color="Sector", title= "PCoA (bray) of TOX Soil Bacterial Communitites: 6000 ADH") + geom_point(aes(color = Sector), stroke=1.5, alpha = 1.5, size=4)+geom_point(size=1.5)+theme_bw()
```
...
perm2=adonis(bray_soil_6000~BMI, data=trts.soil_6000, permutations=1000, by="margin")
...
## Relative abundance of taxa in Random Forest models
```{r}
#phylum
TOX_phylum = TOX %>%
  tax_glom(taxrank="Phylum") %>%
  ...
  #Firmicutes
firm_p = TOX_phylum %>% filter(Sample_Type == "soil") %>% filter(Phylum == "Firmicutes")
firm = ggplot(subset(firm_p, Trt %in% c("donor")), aes(x=ADH_10_actual, y=Abundance, color=Donor, shape=Trt)) +geom_point(size=2) + scale_color_manual(values=colpalette2)+ theme_bw()

  #Nitrospirae
nitro_p = TOX_phylum %>% filter(Sample_Type == "soil") %>% filter(Phylum == "Nitrospirae")
nitro = ggplot(subset(nitro_p, Trt %in% c("donor")), aes(x=ADH_10_actual, y=Abundance, color=Donor, shape=Trt)) +geom_point(size=2) + scale_color_manual(values=colpalette2)+ theme_bw()
...
TOX_Order = TOX %>%
  ...
  #Clostridiales
clos_o = TOX_Order %>% filter(Sample_Type == "soil") %>% filter(Order == "Clostridiales")
clos = ggplot(subset(clos_o, Trt %in% c("donor")), aes(x=ADH_10_actual, y=Abundance, color=Donor, shape=Trt)) +geom_point(size=2) + scale_color_manual(values=colpalette2)+ theme_bw()

  #Cardiobacteriales
cardio_o = TOX_Order %>% filter(Sample_Type == "soil") %>% filter(Order == "Cardiobacteriales")
cardio = ggplot(subset(cardio_o, Trt %in% c("donor")), aes(x=ADH_10_actual, y=Abundance, color=Donor, shape=Trt)) +geom_point(size=2) + scale_color_manual(values=colpalette2)+ theme_bw()

#plot in one figure, including fungal order pleosporales. To get this to work, you will have to run the code for graphing pleosporales relative abundace in 'TOX2 ITS.Rmd'
library(cowplot)

#with pleo
plot_grid(firm, nitro, clos, cardio, pleo, ncol=2, labels = "AUTO")

#without pleo
plot_grid(firm, nitro, clos, cardio, ncol=2, labels = "AUTO")
```
