...
######################################################
mothur_data= import_mothur(mothur_shared_file = here("Mason_SoilMicrobialEcology-Decomp-BMI_mSphere_2022/TOX2_16S.shared"), mothur_constaxonomy_file = here("Mason_SoilMicrobialEcology-Decomp-BMI_mSphere_2022/TOX2_16S.taxonomy"))
...
write_xlsx(bact.prune.n_meta_otu, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_meta_prune_OTUtable.xlsx"))
write_xlsx(bact.prune.n_OTU_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_prune_OTU_taxtable.xlsx")) 
...
write_xlsx(bact.n_meta_p, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_meta_Phylum_table.xlsx") )
write_xlsx(bact.n_p_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_Phylum_taxtable.xlsx") )
...
write_xlsx(bact.n_meta_c, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_meta_Class_table.xlsx") )
write_xlsx(bact.n_c_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_Class_taxtable.xlsx"))
...
write_xlsx(bact.n_meta_o, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_meta_Order_table.xlsx") )
write_xlsx(bact.n_o_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_16S_TSS_Order_taxtable.xlsx"))

########################################################
### obtain ITS OTU table from phyloseq object ##########
#######################################################
mothur_data_ITS= import_mothur(mothur_shared_file =here("Mason_SoilMicrobialEcology-Decomp-BMI_mSphere_2022/TOX2_ITS.shared"), mothur_constaxonomy_file =here("Mason_SoilMicrobialEcology-Decomp-BMI_mSphere_2022/TOX2_ITS.taxonomy"))
moth_merge_ITS = merge_phyloseq(mothur_data_ITS, map) #merge mothurdata with the metadata
colnames(tax_table(moth_merge_ITS)) #print column names of tax file
colnames(tax_table(moth_merge_ITS)) = c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species") #rename tax column names
moth_sub_ITS=subset_samples(moth_merge_ITS, Type == "sample") #remove check samples
moth_soil_ITS=subset_samples(moth_merge_ITS, Trt == "donor") #remove fluid samples and control soil samples
moth_soil_ITS=subset_samples(moth_soil_ITS, group != "TOX002_1500A") #remove TOX002-1500
moth_soil_ITS=subset_samples(moth_soil_ITS, group != "TOX002_1500B") #remove TOX002-1500

ITS_phylo = moth_soil_ITS

#replace 'otu' in ITS dataframe with 'ITS' to know difference between 16S and ITS for merged otu table
taxa_names(ITS_phylo) = gsub("Otu", "ITS", taxa_names(ITS_phylo))

####### ITS - OTU level
#################################################
ITS_phylo_n=transform_sample_counts(ITS_phylo, function(x) {x/sum(x)*10000})#need to do total sum scaling: aka take relative abundance and then multiply by fixed library size of 10,000
ITS_phylo_n=prune_taxa(taxa_sums(ITS_phylo_n) > 10, ITS_phylo_n)#remove normalized OTUs with less than 10 reads across all samples 
#note went from 26310 taxa to 2219 taxa

ITS_OTU_n=as(otu_table(ITS_phylo_n), "matrix")
if(taxa_are_rows(ITS_phylo_n)){ITS_OTU_n=t(ITS_OTU_n)}
ITS_OTU_n_df=as.data.frame(ITS_OTU_n)
ITS_OTU_n_df$group = row.names(ITS_OTU_n_df)

ITS.n_meta_otu = metadata %>% left_join(ITS_OTU_n_df, by = "group")%>%filter(ITS00001 != "NA")

ITS.n_OTU_tax_table = as.data.frame(tax_table(ITS_phylo_n))
ITS.n_OTU_tax_table$OTU = row.names(ITS.n_OTU_tax_table) #make a new row names "OTU" with otu names
write_xlsx(ITS.n_meta_otu, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_meta_OTU_table.xlsx") )
write_xlsx(ITS.n_OTU_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_OTU_taxtable.xlsx") )


####### ITS - Phylum level
################################
ITS_phylum = ITS_phylo %>%
  tax_glom(taxrank="Phylum")
ITS_phylum_n=transform_sample_counts(ITS_phylum, function(x) {x/sum(x)*10000})
ITS_phylum_n=prune_taxa(taxa_sums(ITS_phylum_n) > 10, ITS_phylum_n)
#note: went from 19 to 16 taxa

ITS_phylum_n_table=as(otu_table(ITS_phylum_n), "matrix")
if(taxa_are_rows(ITS_phylum_n)){ITS_phylum_n_table=t(ITS_phylum_n_table)}
ITS_phylum_n_df=as.data.frame(ITS_phylum_n_table)
ITS_phylum_n_df$group = row.names(ITS_phylum_n_df)

ITS.n_meta_p = metadata %>% left_join(ITS_phylum_n_df, by = "group")%>%filter(ITS00002 != "NA")

ITS.n_p_tax_table = as.data.frame(tax_table(ITS_phylum_n))
ITS.n_p_tax_table$OTU = row.names(ITS.n_p_tax_table) #make a new row names "OTU" with otu names
write_xlsx(ITS.n_meta_p, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_meta_Phylum_table.xlsx"))
write_xlsx(ITS.n_p_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_Phylum_taxtable.xlsx")) 

####### ITS - Class level
######################################
ITS_class = ITS_phylo %>%
  tax_glom(taxrank="Class")
ITS_class_n=transform_sample_counts(ITS_class, function(x) {x/sum(x)*10000})
ITS_class_n=prune_taxa(taxa_sums(ITS_class_n) > 10, ITS_class_n)
#note: went from 85 to 66 taxa

ITS_class_n_table=as(otu_table(ITS_class_n), "matrix")
if(taxa_are_rows(ITS_class_n)){ITS_class_n_table=t(ITS_class_n_table)}
ITS_class_n_df=as.data.frame(ITS_class_n_table)
ITS_class_n_df$group = row.names(ITS_class_n_df)

ITS.n_meta_c = metadata %>% left_join(ITS_class_n_df, by = "group")%>%filter(ITS00002 != "NA")

ITS.n_c_tax_table = as.data.frame(tax_table(ITS_class_n))
ITS.n_c_tax_table$OTU = row.names(ITS.n_c_tax_table) #make a new row names "OTU" with otu names
write_xlsx(ITS.n_meta_c, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_meta_Class_table.xlsx") )
write_xlsx(ITS.n_c_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_Class_taxtable.xlsx") )

####### ITS - Order level
######################################
ITS_order = ITS_phylo %>%
  tax_glom(taxrank="Order")
ITS_order_n=transform_sample_counts(ITS_order, function(x) {x/sum(x)*10000})
ITS_order_n=prune_taxa(taxa_sums(ITS_order_n) > 10, ITS_order_n)
#note: went from 219 to 147 taxa

ITS_order_n_table=as(otu_table(ITS_order_n), "matrix")
if(taxa_are_rows(ITS_order_n)){ITS_order_n_table=t(ITS_order_n_table)}
ITS_order_n_df=as.data.frame(ITS_order_n_table)
ITS_order_n_df$group = row.names(ITS_order_n_df)

ITS.n_meta_o = metadata %>% left_join(ITS_order_n_df, by = "group")%>%filter(ITS00002 != "NA")

ITS.n_o_tax_table = as.data.frame(tax_table(ITS_order_n))
ITS.n_o_tax_table$OTU = row.names(ITS.n_o_tax_table) #make a new row names "OTU" with otu names
write_xlsx(ITS.n_meta_o, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_meta_Order_table.xlsx"))
write_xlsx(ITS.n_o_tax_table, here("Mason_SoilMicrobe-PMI_PLoS_2024/data/TOX2_ITS_TSS_Order_taxtable.xlsx"))
