...
```{r, include = FALSE}
#load the datasets:
all_rf_results = readRDS(here("data/all_rf_results.RDS"))
```
...
mae_marker_box <- ggplot(mean_values) +
  aes(x = taxonomy, y = MAE, fill = "indianred") +
  geom_boxplot() +
  ...
  stat_compare_means(method = "anova", hjust = 0.45, vjust = -6) +
...
mae_taxlevel_box <- ggplot(mean_values) +
  aes(x = factor(tax_level, ordered = TRUE, levels = c("Phylum", "Class", "Order", "OTU")), y = MAE, fill = "indianred") +
  geom_boxplot() +
  labs(x = "Taxonomic Level",
  ...
  theme(panel.grid.major = element_blank(),
        ...
        strip.background = element_rect(fill = "white")) +
  scale_fill_discrete(guide = "none") +
  stat_compare_means(method = "anova", label.x.npc = "center", hjust = 0.38, vjust = .2)
...
```{r, include=FALSE, eval=FALSE}
tiff(here::here("figures/Fig4.tiff"), units = "in", width = 5.5, height = 3.5, res = 300)
mae_taxlevel_box
```
...
plot_top_25_features <- function(top_25_df) {
  ...
  top_25_plot <- top_25_df %>%
    ...
    geom_col() +
    coord_flip() + 
    scale_fill_manual(name = "types", values = colors) +
...
```{r, echo=FALSE}
ggarrange(top_bact_mod_features, top_its_mod_features, top_bact_ITS_mod_features,  nrow = 2, ncol = 2, labels = c("A", "B", "C"), align = "v")
```
...
taxa <- c("Firmicutes", "Acidobacteria", "Proteobacteria", "Nitrospirae")

taxa_otus <- bact_phylum_env_top_features$names[which(bact_phylum_env_top_features$tax %in% taxa)]
