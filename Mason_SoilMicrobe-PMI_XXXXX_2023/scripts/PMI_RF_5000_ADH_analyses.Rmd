...
dataset <- data.frame(mod = c(top_mods[1], top_mods[1], top_mods[2], top_mods[2], top_mods[3], top_mods[3]),
           dataset = c("Train", "Test", "Train", "Test", "Train", "Test"), 
           val1 = c(2, 3, 5, 2, 3, 5),
           val2 = c(1:6))
dataset$dataset <- as.factor(dataset$dataset)
t <- ggplot(dataset) +
  aes(x = val1, y = val2, linetype = dataset) +
  geom_point(aes(color = mod)) +
  scale_color_manual(values = c("bact_phylum_env" = 'red', "bact_ITS_order_noenv" = 'blue', "ITS_order_env" = "magenta4"), drop = FALSE) +
...
anova_dat <- data.frame(text = c(paste("ANOVA~italic('p')~`=`", round(anova(mae_16S_tax_level)$`Pr(>F)`, 3)[1], sep = "~"),
                                 paste("ANOVA~italic('p')~`=`", round(anova(mae_mixed_tax_level)$`Pr(>F)`, 3)[1], sep = "~"),
                                 paste("ANOVA~italic('p')~`=`", round(anova(mae_ITS_tax_level)$`Pr(>F)`, 3)[1], sep = "~")),
