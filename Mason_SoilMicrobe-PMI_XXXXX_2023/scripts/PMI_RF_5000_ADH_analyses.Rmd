...
# bquote(paste("ANOVA ", italic("p"), " = ", .(round(anova(mae_16S_tax_level)$`Pr(>F)`, 3)[1]))),
#                                  bquote(paste("ANOVA ", italic("p"), " `=` ", .(round(anova(mae_mixed_tax_level)$`Pr(>F)`, 3)[1]))),
#                                  bquote(paste("ANOVA ", italic("p"), " = ", .(round(anova(mae_ITS_tax_level)$`Pr(>F)`, 3)[1])))

# paste("ANOVA~italic('p')~=~", round(anova(mae_16S_tax_level)$`Pr(>F)`, 3)[1])

# bquote(paste("ANOVA ", italic("p"), " = ", .(round(mae_marker_anova_p$`Pr(>F)`[1], 4))))

# bquote(ANOVA .(italic("p") = .(round(anova(mae_16S_tax_level)$`Pr(>F)`[1], 3))))
