...
```{r, include = FALSE}
mae_diff_otu_prune <- mean_values %>%
  filter(str_detect(model, "otu") == "TRUE") %>%
  filter(taxonomy == "16S") %>%
  group_by(env) %>%
  summarise(mae_diff = diff(MAE))
```

MAE between pruned (*e.g.*, singletons and doubletons removed prior to TSS norm) and unpruned (TSS norm) 16S OTU level models differed by `r pull(mae_diff_otu_prune[mae_diff_otu_prune$env == "no", "mae_diff"])` for models that did not include environmental data and `r pull(mae_diff_otu_prune[mae_diff_otu_prune$env == "yes", "mae_diff"])` for those that did not. 

## Summarize model performace

```{r, include=FALSE}
mean_values <- mean_values %>%
  filter(tax_level != "prune")
```
...
mean_values %>%
  arrange(MAE) %>%
  mutate(model = str_replace(model, "bact", "16S"),
  ...
  kableExtra::save_kable(., file = here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/all_rf_summary_table.pdf"))
...
mean_values %>%
  ...
  column_spec(7, width = "2cm") %>%
  column_spec(8, width = "2cm") %>%
  kableExtra::save_kable(., file = here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S2Table.pdf"))
```
...
# RF <- all_rf_results[[1]]
extract_lm_results <- function(RF, dataset) {
  if (dataset == "test") {
    d <- RF@test_df
  }
  ...
  lm <- lm(d$ADH_10_actual ~ predict(RF@optimal_model_results$optimal_ranger, d)$predictions)
  anova <- anova(lm)
...
# p <- extract_lm_results(RF, dataset = "train")

all_train_predictions <- map(all_rf_results, extract_lm_results, dataset = "train")
all_test_predictions <-  map(all_rf_results, extract_lm_results, dataset = "test")



# mod <- all_train_predictions[[1]]
return_model_summaries <- function(mod_name, list) {
  
  return(list("Model" = mod_name,
              "Dataset" = list[[mod_name]]$dataset,
              "Adj_r2" = signif(summary(list[[mod_name]]$model_summary)$adj.r.squared, 3),
              "Intercept" = signif(list[[mod_name]]$model_summary$coefficients[[1]], 3),
              "Slope" = signif(list[[mod_name]]$model_summary$coefficients[[2]], 3),
              "p_value" = signif(summary(list[[mod_name]]$model_summary)$coefficients[2, 4], 3)))
  
}
# return_model_summaries("bact_otu_noenv", all_train_predictions)

y <- map_dfr(names(all_train_predictions), return_model_summaries, list = all_train_predictions) %>%
  bind_rows(map_dfr(names(all_test_predictions), return_model_summaries, list = all_test_predictions))
  
y %>%
  summarise(min = min(Adj_r2),
            max = max(Adj_r2), 
            .by = c("Dataset"))
...
y %>% 
  ...
  kbl(.,
    format = "latex",
    col.names = c("Model", "Dataset", "Adjusted $r^{2}$", "Intercept", "Slope", "$p$-value"),
    booktabs = TRUE,
    escape = F) 
  
# %>%
#   save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/all_mods_test-train_lm.pdf"), keep_tex = FALSE)
  
# write_csv(y, here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/all_mods_test-train_lm.csv"))
```
...
y %>% 
  ...
  kbl(.,
    ...
    booktabs = TRUE,
    escape = F) %>%
  save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S3Table.pdf"), keep_tex = FALSE)
```
...
mod1_train_predictions <- ggplot(data = NULL,
  ...
  geom_point(size = 1.5) +
  geom_smooth(method = "lm", linetype = "solid", color = 'red') +
...
mod2_train_predictions <- ggplot(data = NULL,
  ...
  geom_point(size = 1.5) +
  geom_smooth(method = "lm", linetype = "solid", color = 'blue') +
  labs(x = "Predicted ADH",
       y = "Actual ADH") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
...
mod3_train_predictions <- ggplot(data = NULL,
  ...
  geom_point(size = 1.5) +
  geom_smooth(method = "lm", linetype = "solid", color = "magenta4") +
  labs(x = "Predicted ADH",
       y = "Actual ADH") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
...
mod3_test_predictions <- ggplot(data = NULL,
  ...
  geom_point(size = 1.5) +
  geom_smooth(method = "lm", linetype = "dashed", color = 'magenta4') +
  labs(x = "Predicted ADH",
       y = "Actual ADH") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
...
top_mods_labels <- top_mods %>% str_replace(., "bact", "16S")

mod_colors <- c("red", "blue", "magenta4")
names(mod_colors) <- top_mods_labels

dataset <- data.frame(mod = c(top_mods_labels[1], top_mods_labels[1], top_mods_labels[2], top_mods_labels[2], top_mods_labels[3], top_mods_labels[3]),
           dataset = c("Train", "Test", "Train", "Test", "Train", "Test"), 
           val1 = c(2, 3, 5, 2, 3, 5),
           val2 = c(1:6)) %>% 
  mutate(mod = factor(mod, levels = c("16S_phylum_env", "16S_ITS_order_noenv", "ITS_order_env")),
         dataset = factor(dataset))
# dataset$dataset <- as.factor(dataset$dataset)
t <- ggplot(dataset) +
  aes(x = val1, y = val2, linetype = factor(dataset, levels = c("Train", "Test"))) +
  geom_point(aes(color = mod)) +
  scale_color_manual(values = mod_colors, drop = FALSE) +
...
```{r, include=FALSE, eval=FALSE}
tiff(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/Fig2.tiff"), units = "in", width = 7.5, height = 5.5, res = 300)
ggarrange(mod1_train_predictions, mod2_train_predictions, mod3_train_predictions, mod1_test_predictions, mod2_test_predictions, mod3_test_predictions,
          nrow = 2,
          ncol = 3,
          labels = c("A", "B", "C", "D", "E", "F"),
...
library(rstatix)
library(ggpubr)
mae_tax_posthoc_t <- mean_values %>%
  t_test(MAE ~ taxonomy) %>%
  adjust_pvalue() %>%
  mutate(y.position = c(1010, 1025, 1050))
write_csv(mae_tax_posthoc_t, here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/mae_marker_posthoc_t_results.csv"))
...
mae_marker_box <- ggplot(mean_values) +
  aes(x = taxonomy, y = MAE) +
  geom_boxplot(aes(fill = taxonomy)) +
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  stat_pvalue_manual(mae_tax_posthoc_t, label = "p.adj") +
  # stat_compare_means(comparisons = my_comparisons, method = "t.test", label = "p.format", label.y = c(1010, 1025, 1050)) +
  annotate("text",
           x = 1,
           y = 1070,
           label = bquote(paste("ANOVA ", italic("p"), " = ", .(round(mae_marker_anova_p$`Pr(>F)`[1], 4)))),
           size = 3) 
...
mae_tax_posthoc_t %>%
  ...
  kbl(.,
      format = "latex",
      col.names = c("Group 1", "Group 2", "Statistic", "df", "$p$", "Adjusted $p$"),
      digits = 3,
      booktabs = TRUE,
      escape = FALSE) %>% 
  save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/MAE_biologicalMarker_posthoct.pdf"), keep_tex = FALSE)
...
mae_tax_posthoc_t %>%
  ...
  kbl(.,
      ...
      booktabs = TRUE,
      escape = FALSE) %>% 
  save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S5Table.pdf"), keep_tex = FALSE)
```
...
kbl(mae_marker_anova_p,
    ...
    digits = 3,
    booktabs = TRUE) %>% 
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S4Table.pdf"), keep_tex = FALSE)
```

```{r, include=FALSE, eval=FALSE}
tiff(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/Fig3.tiff"), units = "in", width = 4, height = 4, res = 300)
mae_marker_box
dev.off()
```
...
```{r, include=FALSE, eval = FALSE}
mae_taxlevel_anova_p <- anova(lm(MAE ~ tax_level, data = mean_values))

rownames(mae_taxlevel_anova_p)[1] <- "taxonomicLevel"
kbl(mae_taxlevel_anova_p,
    format = "latex",
    row.names = TRUE,
    digits = 3,
    booktabs = TRUE) %>% 
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/MAE_taxlevel_anova.pdf"), keep_tex = FALSE)
```
...
kbl(mae_taxlevel_anova_p,
    ...
    digits = 3,
    booktabs = TRUE) %>% 
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S6Table.pdf"), keep_tex = FALSE)
```

```{r, include=FALSE}
anova(lm(MAE ~ tax_level*taxonomy, data = mean_values))
ggplot(mean_values) +
  aes(y = MAE, x = factor(tax_level, ordered = TRUE, levels = c("Phylum", "Class", "Order", "OTU")), color = taxonomy, group = taxonomy) +
  geom_point() +
...
mae_16S_tax_level_anova <- data.frame(anova(mae_16S_tax_level))[1, ] %>%
  rownames_to_column(var = "bio_marker") %>%
  mutate(bio_marker = recode(bio_marker, "tax_level" = "16S"))


mae_ITS_tax_level <- filter(mean_values, taxonomy == "ITS") %>% lm(MAE ~ tax_level, data = .)
anova(mae_ITS_tax_level)$`Pr(>F)`[1]
mae_ITS_tax_level_anova <- data.frame(anova(mae_ITS_tax_level))[1, ] %>%
  rownames_to_column(var = "bio_marker") %>%
  mutate(bio_marker = recode(bio_marker, "tax_level" = "ITS"))

mae_mixed_tax_level <- filter(mean_values, taxonomy == "16S-ITS") %>% lm(MAE ~ tax_level, data = .)
anova(mae_mixed_tax_level)$`Pr(>F)`[1]
mae_16SITS_tax_level_anova <- data.frame(anova(mae_mixed_tax_level))[1, ] %>%
  rownames_to_column(var = "bio_marker") %>%
  mutate(bio_marker = recode(bio_marker, "tax_level" = "16S-ITS"))
```

```{r, include=FALSE}  
rbind(mae_16S_tax_level_anova, mae_16SITS_tax_level_anova, mae_ITS_tax_level_anova) %>%
  rename("Biological Marker" = "bio_marker",
         "F value" = "F.value",
        `Pr(>F)` = "Pr..F.") %>%
  kbl(.,
    format = "latex",
    digits = 3,
    booktabs = TRUE) %>% 
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/MAE_taxlevel_biomarker_anova.pdf"), keep_tex = FALSE)
```
...
rbind(mae_16S_tax_level_anova, mae_16SITS_tax_level_anova, mae_ITS_tax_level_anova) %>%
  ...
  kbl(.,
    ...
    digits = 3,
    booktabs = TRUE) %>% 
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S7Table.pdf"), keep_tex = FALSE)
```
...
mean_values$tax_level <- factor(mean_values$tax_level, ordered = TRUE, levels = c("Phylum", "Class", "Order", "OTU"))
pairwise.t.test(mean_values[mean_values$taxonomy == "16S" , "MAE"], mean_values[mean_values$taxonomy == "16S" , "tax_level"], p.p.adjust.method = "none")

diff(range(mean_values[mean_values$taxonomy == "16S" , "MAE"]))

mean_values %>%
  filter(taxonomy == "16S") %>%
  summarise(mean_mae = mean(MAE), .by = c("tax_level"))
  summarise(range = diff(range(MAE)), .by = c("tax_level")) %>%
  summarise(mean_range = mean(range)) #26.9072

mean_values %>%
  filter(taxonomy == "16S") %>%
  t_test(MAE ~ tax_level) %>%
  adjust_pvalue()

mae_ITS_tax_level <- filter(mean_values, taxonomy == "ITS") %>% lm(MAE ~ tax_level, data = .)
anova(mae_ITS_tax_level) %>% writexl::write_xlsx(., here::here("data/MAE_ITS_taxlevel_anova.xlsx"))
pairwise.t.test(mean_values[mean_values$taxonomy == "ITS" , "MAE"], mean_values[mean_values$taxonomy == "ITS" , "tax_level"], p.p.adjust.method = "none")

diff(range(mean_values[mean_values$taxonomy == "ITS" , "MAE"]))
...
pairwise.t.test(mean_values[mean_values$taxonomy == "16S-ITS" , "MAE"], mean_values[mean_values$taxonomy == "16S-ITS" , "tax_level"], p.p.adjust.method = "none")

diff(range(mean_values[mean_values$taxonomy == "16S-ITS" , "MAE"]))
...
```{r, include=FALSE, eval=FALSE}
tiff(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/Fig4.tiff"), units = "in", width = 6, height = 3.5, res = 300)
mae_taxlevel_box
dev.off()
```
...
get_top_25_features <- function(optimal_model, tax_table, tax_level) {
  ...
  top_25_tax <- tax_table %>%
    filter(OTU %in% top_25$names) 

  top_25 <- top_25 %>%
    left_join(top_25_tax, by = c("names" = "OTU")) %>%
    mutate(names = case_when(names == "EC_LRR" ~ "Electrical conductivity",
...
```{r, include=FALSE, warning=FALSE}
bact_phylum_tax_table <- read_excel(here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/TOX2_16S_TSS_Phylum_taxtable.xlsx"))
bact_phylum_env_imp <- get_top_25_features(model_1_opt_ranger, bact_phylum_tax_table, "Phylum") %>%
  mutate(model = "16S phylum env",
         Species = NA_character_) %>%
  select(model, names, x, predictor_type, Kingdom, Phylum, Class, Order, Family, Genus, Species) 
top_bact_mod_features <- get_top_25_features(model_1_opt_ranger, bact_phylum_tax_table, "Phylum") |>
  plot_top_25_features()


bact_order_tax_table <- read_excel(here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/TOX2_16S_TSS_Order_taxtable.xlsx"))
its_order_tax_table <- read_excel(here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/TOX2_ITS_TSS_Order_taxtable.xlsx"))

combined_order_tax_table <- bind_rows(bact_order_tax_table, its_order_tax_table)
bact_its_order_imp <- get_top_25_features(model_2_opt_ranger, combined_order_tax_table, "Order") %>%
  mutate(model = "16S-ITS order noenv") %>%
  select(model, names, x, predictor_type, Kingdom, Phylum, Class, Order, Family, Genus, Species) 
top_bact_ITS_mod_features <- get_top_25_features(model_2_opt_ranger, combined_order_tax_table, "Order") |>
  plot_top_25_features()

its_order_env_imp <- get_top_25_features(model_3_opt_ranger, its_order_tax_table, "Order") %>%
  mutate(model = "ITS order env") %>%
  select(model, names, x, predictor_type, Kingdom, Phylum, Class, Order, Family, Genus, Species) 
...
```{r, include=FALSE, eval=FALSE}
top_mods_imp <- rbind(bact_phylum_env_imp,
                      bact_its_order_imp,
                      its_order_env_imp)  %>%
  group_by(model) %>%
  arrange(desc(x))
  
which(top_mods_imp$model %in% "16S phylum env") 
which(top_mods_imp$model %in% "16S-ITS order noenv") 
which(top_mods_imp$model %in% "ITS order env") 

options(knitr.kable.NA = '')

top_mods_imp %>%
  select(-model, -Family, -Genus, -Species) %>%
  kbl(.,
      format = "latex",
      col.names = c("Feature", "Importance", "Type", "Kingdom", "Phylum", "Class", "Order"),
      digits = 1,
      booktabs = T) %>%
  pack_rows("16S phylum env", 1, 25, latex_gap_space = "1em") %>%
  pack_rows("16S-ITS order noenv", 26, 50, latex_gap_space = "1em") %>%
  pack_rows("ITS order env", 51, 75, latex_gap_space = "1em") %>%
  save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/top_models_featureimportance.pdf"))
...
top_mods_imp %>%
  ...
  pack_rows("16S-ITS order noenv", 26, 50, latex_gap_space = "1em") %>%
  pack_rows("ITS order env", 51, 75, latex_gap_space = "1em") %>%
  save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S8Table.pdf"))
...
phyla_taxa <- bact_phylum_env_top_features %>%
  filter(predictor_type == "16S") %>%
  slice_head(n = 5) %>%
  pull(tax)

# phyla_taxa <- c("Firmicutes", "Acidobacteria", "Proteobacteria", "Nitrospirae")

phyla_taxa_otus <- bact_phylum_env_top_features$names[which(bact_phylum_env_top_features$tax %in% phyla_taxa)]
...
ggplot(subset(phylum_abundance, Phylum %in% phyla_taxa[1])) +
  aes(x = ADH_10_actual, y = Abundance, color = Donor) +
  geom_point() 

theme_abundance(ggplot(subset(phylum_abundance, Phylum %in% phyla_taxa[1])) +
  aes(x = ADH_10_actual, y = Abundance, color = Donor) +
  geom_point() )
...
```{r, include=FALSE, eval=FALSE}
tiff(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/Fig6.tiff"), units = "in", width = 7.5, height = 5.5, res = 300)
phylum_plot
dev.off()
```

```{r, include=FALSE, eval=FALSE}
timepoint_counts <- metadata %>%
  filter(Trt == "donor") %>%
  filter(ADH_10_actual <= 5000) %>%
  group_by(Donor) %>%
  count()
...
metadata_table <- metadata %>%
  ...
  filter(ADH == 0) %>%
  arrange(Donor) %>%
  left_join(timepoint_counts, by = "Donor") %>%
  select(Donor, Sex, Age, Weight, Season, Total_ADH, n) %>%
  mutate(Sex = case_when(Sex == "F" ~ "Female", Sex == "M" ~ "Male"),
         Season = case_when(Season == "F" ~ "Fall", Season == "W" ~ "Winter", Season == "Sp" ~ "Spring", Season == "Su" ~ "Summer"))
  
kbl(metadata_table,
    format = "latex",
    col.names = c("Donor", "Age", "Sex", "Weight", "Season", "Total ADH", "Timpoints in Models"),
    digits = 3,
    booktabs = TRUE,
    align = 'c') %>%
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/Donor_metadata.pdf"), keep_tex = FALSE)
```
...
kbl(metadata_table,
    ...
    booktabs = TRUE,
    align = 'c') %>%
save_kable(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S1Table.pdf"), keep_tex = FALSE)
```

```{r, include=FALSE, warning=FALSE}
its_order_env_top_features <- get_top_25_features(all_rf_results$ITS_order_env@optimal_model_results$optimal_ranger, its_order_tax_table, "Order")

its_order_env_top_features %>%
  filter(predictor_type == "ITS") %>%
  slice_head(n = 5) %>%
  pull(tax)

taxa <- c("o__Pleosporales")

taxa_otus <- its_order_noenv_top_features$names[which(its_order_env_top_features$tax %in% taxa)]
...
```{r, include = FALSE}
bact_its_order_env_top_features <- get_top_25_features(all_rf_results$ITS_order_noenv@optimal_model_results$optimal_ranger, its_order_tax_table, "Order")

bact_order_tax_table <- read_excel(here("Mason_SoilMicrobe-PMI_XXXXX_2023/data/TOX2_16S_TSS_Order_taxtable.xlsx"))


bact_its_order_top_features <- get_top_25_features(all_rf_results$bact_ITS_order_noenv@optimal_model_results$optimal_ranger, bact_order_tax_table, "Order")

order_taxa <- bact_its_order_top_features %>%
  filter(predictor_type == "16S") %>%
  slice_head(n = 5) %>%
  pull(tax)
...
```{r, include=FALSE, eval=FALSE}
tiff(here::here("Mason_SoilMicrobe-PMI_XXXXX_2023/figures/S2Fig.tiff"), units = "in", width = 7.5, height = 5.5, res = 300)
order_plot
dev.off()
```
